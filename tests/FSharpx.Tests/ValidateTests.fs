module FSharpx.Tests.ValidateTests
...
let validator pred error value =
    if pred value
        then Ok value
        else Error (NonEmptyList.singleton error)
...
let validateOrder (o: Order) =
    ...
    nameNotNull >>= positiveCost |> Result.map (konst o)
...
let ``using ap``() =
  ...
  let result =
    ...
    |> Validate.ap (nonNull "Surname can't be null" customer.Surname)
    |> Validate.ap (notEqual "foo" "Surname can't be foo" customer.Surname)
  match result with
  | Ok c -> failwithf "Valid customer: %A" c
  | Error errors ->
      printfn "Invalid customer. Errors:\n%A" errors
...
let ``validation with sum monoid``() =
    let v = Validate.CustomValidation (Monoid.sum())
...
let ``using sequenceIgnore``() =
    ...
    let vError = Validate.sequenceIgnore vsError
    match vError with
    | Error errors ->
    ...
    let vOk = Validate.sequenceIgnore vsOk
    match vOk with
    | Ok () -> ()
...
let ``using mapMIgnore`` =
    ...
    let vError = Validate.mapMIgnore validate okAndErr
    let vOk = Validate.mapMIgnore validate oks
