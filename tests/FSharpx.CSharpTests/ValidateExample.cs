...
namespace FSharpx.CSharpTests.ValidateExample {
    ...
    class Club {
        public static readonly Func<Person, FSharpResult<Person, NonEmptyList<string>>>
            CheckAge =
                p => {
                    if (p.Age < 18)
                        return FSharpResult.Error<Person>("Too young!");
                    if (p.Age > 40)
                        return FSharpResult.Error<Person>("Too old!");
                    return FSharpResult.Ok(p);
                };
            ...
            CheckClothes =
                p => {
                    if (p.Gender == Gender.Male && !p.Clothes.Contains("Tie"))
                        return FSharpResult.Error<Person>("Smarten up!");
                    if (p.Gender == Gender.Female && p.Clothes.Contains("Trainers"))
                        return FSharpResult.Error<Person>("Wear high heels!");
                    return FSharpResult.Ok(p);
                };
            ...
            CheckSobriety =
                p => {
                    if (new[] { Sobriety.Drunk, Sobriety.Paralytic, Sobriety.Unconscious }.Contains(p.Sobriety))
                        return FSharpResult.Error<Person>("Sober up!");
                    return FSharpResult.Ok(p);
                };
    }
    ...
    class ClubbedToDeath {
        ...
        public static FSharpResult<decimal, NonEmptyList<string>> CostToEnter(Person p) {
    ...
    class Test1 {
        ...
        public void Part1() {
            ...
            Assert.AreEqual(FSharpResult.Error<decimal>("Too old!"), costDave);

            var costKen = ClubbedToDeath.CostToEnter(Ken);
            Assert.AreEqual(FSharpResult.Ok(5m), costKen);

            var costRuby = ClubbedToDeath.CostToEnter(Ruby);
            Assert.AreEqual(FSharpResult.Ok(0m), costRuby);
            ...
            Assert.AreEqual(FSharpResult.Error<decimal>("Too young!"), costRuby17);
            ...
            Assert.AreEqual(FSharpResult.Error<decimal>("Sober up!"), costKenUnconscious);
    ...
    class ClubTropicana {
        ...
        public static FSharpResult<decimal, NonEmptyList<string>> CostToEnter2(Person p) {
            return CostByGender.Curry().ReturnValidate()
                .ApValidate(Club.CheckAge(p))
                .ApValidate(Club.CheckClothes(p))
                .ApValidate(Club.CheckSobriety(p));
        }
    }
    ...
    class Test2 {
        ...
        public void Part2() {
            ...
            Assert.AreEqual(FSharpResult.Errors<decimal>("Too old!", "Sober up!"), costDaveParalytic);
    ...
    class Test3 {
        ...
        public void Part3() {
            ...
            Assert.AreEqual(FSharpResult.Errors<decimal>("Too old!", "Smarten up!", "Sober up!"), cost);
        }
    }
}
