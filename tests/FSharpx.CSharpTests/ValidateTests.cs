    ...
    public class ValidateTests {
        ...
        public void Test1_either() {
            ...
            var somethingOrError =
                from userID in FSharpOption.ParseInt(req_userID).ToFSharpResult("Invalid User ID")
                from id in FSharpOption.ParseInt(req_otherID).ToFSharpResult("Invalid ID")
        ...
        public void Test2_either() {
            var userID = FSharpOption.ParseInt(req_userID)
                .ToFSharpResult(NonEmptyList.Singleton("Invalid User ID"));
            var id = FSharpOption.ParseInt(req_otherID)
                .ToFSharpResult(NonEmptyList.Singleton("Invalid ID"));

            var doSomethingFunc = L.F((int a, int b) => doSomething(a, b));
            var curriedDoSomething = doSomethingFunc.Curry();
            var result = curriedDoSomething.ReturnValidate()
                .ApValidate(userID)
                .ApValidate(id);
        ...
        static FSharpResult<T, Errors> NonNull<T>(T value, string err) where T: class {
            return FSharpResult.Validator<T>(x => x != null, err)(value);
        }

        static FSharpResult<T, Errors> NotEqual<T>(T value, T other, string err) {
            return FSharpResult.Validator<T>(v => !Equals(v, other), err)(value);
        }

        static FSharpResult<Address, Errors> ValidateAddressLines(Address a) {
            if (a.Line1 != null || a.Line2 == null)
                return FSharpResult.Ok(a);
            return FSharpResult.Error<Address>("Line1 is empty but Line2 is not");
        }

        /// <summary>
        /// Same as FSharpResult.Validator, for demo purposes only
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="pred"></param>
        /// <param name="err"></param>
        /// <returns></returns>
        static Func<T, FSharpResult<T, Errors>> Validator<T>(Predicate<T> pred, string err) {
            return x => {
                if (pred(x))
                    return FSharpResult.Ok(x);
                return FSharpResult.Error<T>(err);
            };
        }

        #pragma warning disable 414
        static Func<Address, FSharpResult<Address, Errors>> ValidateAddressLines2 =
            Validator<Address>(x => x.Line1 != null || x.Line2 == null, "Line1 is empty but Line2 is not");
        #pragma warning restore 414

        static FSharpResult<Address, Errors> ValidateAddress(Address a) {
            return from x in NonNull(a.Postcode, "Post code can't be null")
                   join y in ValidateAddressLines(a) on 1 equals 1
                   select a;
        }

        static FSharpResult<T, Errors> GreaterThan<T>(T value, T other, string err) where T: IComparable<T> {
            ...
            if (valueNull && otherNull || valueNull != otherNull || value.CompareTo(other) > 0)
                return FSharpResult.Ok(value);
            return FSharpResult.Error<T>(err);
        }

        static FSharpResult<T?, Errors> GreaterThan<T>(T? value, T? other, string err) where T: struct, IComparable<T> {
            if (!value.HasValue && !other.HasValue || value.HasValue != other.HasValue || value.Value.CompareTo(other.Value) > 0)
                return FSharpResult.Ok(value);
            return FSharpResult.Error<T?>(err);
        }

        static FSharpResult<Order, Errors> ValidateOrder(Order o) {
            return
        ...
        static FSharpResult<FSharpList<Order>, Errors> ValidateOrders(IEnumerable<Order> orders) {
            var zero = ListModule.Empty<Order>().ReturnValidate();
            return orders
